...
def show_results_random(some_cats):
    ...
    plot_pie(p)
    st.write(
        "P(Rich) = {:.2f}%, with {:,}/{:,} ({:.4f}%) of the entire dataset.".format(
            p, n, N, s
        )
    )
    return df_pop
...
feat_label_choosen = st.sidebar.multiselect(
    "What characteristics?", list(features_label.values())
)
...
if N_choosen > 0:
    ...
    if N_choosen < 2:
        ...
        tmp = "{} and {}:".format(
            ", ".join(feat_label_choosen[:-1]), feat_label_choosen[-1]
        )
    ...
    if st.sidebar.checkbox("Set more details."):
        ...
        for feat in feat_choosen:
            ...
            if feat in features_cat_dict.keys():
                values_feat_choosen[feat] = st.sidebar.selectbox(
                    text_tmp, features_cat_dict[feat]
                )
            if feat in features_num_dict.keys():
                values_feat_choosen[feat] = st.sidebar.slider(
                    text_tmp,
                    features_num_dict[feat][0],
                    features_num_dict[feat][-1],
                    33,
                )
        if len(values_feat_choosen) > 0:
            Fst_subtitle.subheader("Picking for {}".format(tmp))
        if len(values_feat_choosen) > 0:
            Snd_subtitle.dataframe(
                pd.DataFrame(
                    [
                        (features_label[key], val)
                        for key, val in values_feat_choosen.items()
                    ],
                    columns=["Char.", "Value"],
                )
            )
